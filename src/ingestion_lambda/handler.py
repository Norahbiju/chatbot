...
config = Config(retries={"max_attempts": 3, "mode": "standard"}, read_timeout=20, connect_timeout=3)
...
def _start_job(knowledge_base_id: str, data_source_id: str, keys: List[str]) -> str:
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            if not key.startswith(prefix):
                _log("ignored_key", message_id=message_id, key=key)
                continue
    ...
    try:
        if _has_active_job(knowledge_base_id, data_source_id):
            ...
            job_id = _start_job(knowledge_base_id, data_source_id, candidate_keys)
            _log("ingestion_started", changed_objects=len(candidate_keys), ingestion_job_id=job_id)
