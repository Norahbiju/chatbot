...
## Open issues
Changing the S3 Vectors index metadata configuration is a vector-index replacement concern and requires a fresh ingestion after deployment. The new APAC inference-profile path still needs AWS-backed deployment validation in the target account. The exact S3 Vectors ARN format is still derived because the provider resources do not export ARN attributes directly; AWS deployment validation should confirm IAM resource matching.
