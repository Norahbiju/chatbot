...
- `infra/modules/core` for the source document bucket, S3 Vectors, Bedrock KB/data source, SNS, and SSM parameters
...
One Terraform apply now creates the infrastructure system. Internally, the application module receives the knowledge base ID and SNS topic ARN directly from the core module outputs, while the core module still publishes those identifiers to SSM for external consumers. Source document objects are synced by the `Sync Documents` GitHub Actions workflow after infrastructure exists.
...
- 2026-08-01: Removed Terraform-managed source document objects and the automatic ingestion Lambda/EventBridge/SQS path; Knowledge Base content sync is now handled by the `Sync Documents` workflow.
