...
## Validation evidence
`python -m compileall src scripts` passed. `python -m unittest discover -s src/ingestion_lambda/tests`, `python -m unittest discover -s src/query_lambda/tests`, and `python -m unittest discover -s scripts/tests` passed on 2026-07-30 using a local virtual environment with `boto3` and `botocore` installed for the run. Frontend uses DOM creation and `textContent`; no model output is inserted with `innerHTML`. After the July 30, 2026 repair and exact apply, a live API probe returned a Kubernetes answer with four citations from the deployed knowledge base.
