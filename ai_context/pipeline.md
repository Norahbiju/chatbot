...
- `workflow_dispatch` with `action` of `plan`, `apply`, or `destroy`; `stack` of `core` or `application`; `environment` of `dev`; optional `source_run_id`, `pr_number`, and `destroy_confirmation`.
...
GitHub Environments:
- `terraform-dev-plan`
- `terraform-dev-apply` with required reviewers
- `terraform-dev-destroy` with required reviewers
...
- Publishes plan output to the job summary, artifact, and optionally a same-repository PR comment.
...
- Uses environment `terraform-dev-apply` and concurrency group `terraform-dev-<stack>`.
...
- Generates a saved destroy plan, uploads it, then a separate environment-protected job downloads and applies only that saved destroy plan.
- Uses environment `terraform-dev-destroy` and the same stack concurrency group.
...
## Open issues
GitHub-hosted execution is required to validate workflow expressions, OIDC claim behavior, artifact download from source runs, PR sticky comments, and environment approvals. AWS account validation is required for role permissions, state bucket access, S3 lockfile access, Bedrock/S3 Vectors planning, and destroy active-ingestion checks.
